...
parser.add_argument('-mfu', '--manual-flip-unc', action='store_true', default=False)
...
def get_flipped_object_pose_manual(anygrasp_data_dir, cur_pose_actual, flip_transform, nerf2ws_file_expected):
    flip_transform_new = flip_transform.copy()
    flip_transform_new[:3, -1] = cur_pose_actual[:3, -1]*2
    exp_pose = flip_transform @ cur_pose_actual
    x_unc = np.random.uniform(-0.015, 0.015)
    x_unc = np.sign(x_unc) * (np.abs(x_unc) + 0.01)
    y_unc = np.random.uniform(-0.015, 0.015)
    x_unc = np.sign(y_unc) * (np.abs(y_unc) + 0.01)
    rot_unc = random_rotation_matrix(np.random.uniform(2*np.pi/180, 4*np.pi/180)) 
    z_off = 0.05
    start_pose = exp_pose.copy()
    start_pose[:3, -1] += np.array([x_unc, y_unc, z_off])
    
    start_pose_file = f"{anygrasp_data_dir}/start_pose.pkl"
    out_pose_file = f"{anygrasp_data_dir}/object_actual_pose.pkl"
    pickle.dump(start_pose, open(start_pose_file, 'wb'))
    sim_settle_cmd = f"cd /home/saptarshi/.local/share/ov/pkg/isaac_sim-2022.2.1 && PYTHONUNBUFFERED=1 ./python.sh /home/saptarshi/dev/CustomComposer/src/two_robots_with_gravity_settle_pos.py --out-path {out_pose_file} --obj-type {obj_type} --start-pose-file {start_pose_file}"
    subprocess.run(sim_settle_cmd, shell=True, env=my_env)
    
    sim_grasp_cmd = f"cd /home/saptarshi/.local/share/ov/pkg/isaac_sim-2022.2.1 && PYTHONUNBUFFERED=1 ./python.sh /home/saptarshi/dev/CustomComposer/src/simulation_video_generator_two_robots_with_gravity_testing_multi_img_inerf.py -dd {anygrasp_data_dir} -ot {obj_type} -ooe {nerf2ws_file_expected} -ooa {out_pose_file} --dont-flip"
    subprocess.run(sim_grasp_cmd, shell=True, env=my_env)
   
    actual_pose = pickle.load(open(out_pose_file, 'rb'))
...
while iter_start <= total_iter:
    ...
    if iter_start > 0:
        ...
        if flip_transform is not None:
            ...
            if not args.manual_flip_unc:
                cur_pose_actual = get_flipped_object_pose(anygrasp_data_dir, pose_file_actual, nerf2ws_file_expected)
            else:
                cur_pose_actual = get_flipped_object_pose_manual(anygrasp_data_dir, cur_pose_actual, flip_transform, nerf2ws_file_expected)
            ...
            for idx in range(1, num_caliberation_images+1):
                ...
                if sam_file_to_use == 'sam_backsub_custom_no_morph.sh':
                    subprocess.run(f"/home/saptarshi/dev/nerf-pytorch/sim_data/sam_backsub_custom_single_img_no_morph.sh {post_flip_img_path} \"{obj_prompt}\" {inerf_dir}",shell=True, env=my_env)
                else:
    ...
    if len(captured_imgs) == 0:
        ...
        if iter_start != 0:
            tot_valid_iters += 1
        print(f"Total Valid Iterations Ran till Now (Excluding 0th) :", tot_valid_iters)
