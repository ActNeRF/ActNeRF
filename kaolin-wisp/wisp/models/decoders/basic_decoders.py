...
class BasicDecoder(WispModule):
    ...
    def make(self):
        ...
        # for i in range(self.num_layers):
        #     if i == 0: 
        #         layers.append(self.get_layer(self.input_dim, self.hidden_dim, bias=self.bias))
        #     elif i in self.skip:
        #         layers.append(self.get_layer(self.hidden_dim+self.input_dim, self.hidden_dim, bias=self.bias))
        #     else:
        #         layers.append(self.get_layer(self.hidden_dim, self.hidden_dim, bias=self.bias))
        # self.layers = nn.ModuleList(layers)
        # self.lout = self.get_layer(self.hidden_dim, self.output_dim, bias=self.bias)

    def get_layer(self, in_dim, out_dim, bias):
        layer = self.layer(in_dim, out_dim, bias=bias)
        torch.nn.init.xavier_uniform_(layer.weight, 2)
        return layer
