...
from wisp.ops.image import write_png, write_exr, write_epistemic
...
class MultiviewTrainer(BaseTrainer):
    ...
    def evaluate_metrics(self, dataset: MultiviewDataset, lod_idx, name=None, lpips_model=None):
        ...
        with torch.no_grad():
            for idx, full_batch in tqdm(enumerate(dataset)):
                ...
                # write_epistemic(os.path.join(self.valid_epistemic_log_dir, out_name + ".npy"), rb.epistemic.cpu().numpy())
                # print(rb.shape)
                # save_path_img = os.path.join(self.valid_log_dir, "depth_" + out_name + ".png")
                # save_path_pkl = os.path.join(self.valid_log_dir, "depth_" + out_name + ".pkl")
                # depth_image = rb.cpu().depth.numpy()[:,:,0]
                # alpha = rb.alpha.detach().cpu().numpy()[:,:,0]
                # depth_image[alpha <= 0.1] = 10
                # depth_image[alpha > 0.1] = depth_image[alpha > 0.1] / alpha[alpha > 0.1]
                # Image.fromarray((depth_image * 1000).astype(np.int32), 'I').save(save_path_img)
                # pickle.dump(depth_image, open(save_path_pkl, 'wb'))
    ...
    def validate(self):
        ...
        self.valid_epistemic_log_dir = os.path.join(self.log_dir, "epistemic_val")
        ...
        if not os.path.exists(self.valid_epistemic_log_dir):
            os.makedirs(self.valid_epistemic_log_dir)
