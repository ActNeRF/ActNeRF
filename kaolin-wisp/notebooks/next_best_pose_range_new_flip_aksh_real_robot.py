...
base_scale = 2
base_offset = torch.tensor([-2.0, 0, -0.2])
...
def get_flip_transpose_np(object_center):
    p = np.eye(4)
    p[:3, :3] = flip_rot
    p[:3, 3] = -(flip_rot @ object_center.numpy()) + object_center.numpy()
    return p
    
def get_flip_transpose_torch_cpu(object_center):
    p_np = get_flip_transpose_np(object_center)
    p = torch.tensor(p_np)
    return p

def get_flip_transpose_torch_gpu(object_center):
    p = get_flip_transpose_torch_cpu(object_center)
    return p.cuda()


def main(output_dump_dir, ranges, pipelines, object_center, robot_poses, mean_unc, nerf2ws, move_lambda, unc_lambda):
    print("running main at nerf2ws : ", nerf2ws)    
    ws2nerf = torch.inverse(nerf2ws)
    range_x = ranges[0]
    range_y = ranges[1]
    range_z = ranges[2]
    ...
    # reverse_y is for indigenous robot, it is easier to reach positions with the camera Y-axis
    # pointing downward as the arm is placed higher than the object
    def get_c2w_from_pos(camera_position: np.ndarray, lookat: np.ndarray, reverse_y: bool = F) -> torch.tensor:
    ...
    if robot_poses is not None:
        ...
        cur_pose = get_c2w_from_pos(np.array([0.95, 0.4, 0.4]), object_center.numpy())
    ...
    def in_robot_range(c2w_scaled, forced_nerf2ws=None):
        xyz_scaled = c2w_scaled[..., :3, 3].squeeze()
        xyz = (xyz_scaled.detach().cpu() - translation_offset) / translation_scale

        if forced_nerf2ws is not None:
            xyz = torch.matmul(forced_nerf2ws.to(xyz.device).double()[:3, :3], xyz.double()) + forced_nerf2ws.to(xyz.device).double()[:3, 3]
        else:
            xyz = torch.matmul(nerf2ws.to(xyz.device).double()[:3, :3], xyz.double()) + nerf2ws.to(xyz.device).double()[:3, 3]
            
        # some hardcoding for real robot
        min_x = 0.85
        max_y = 0.4
        min_y = -0.4
        # breakpoint()
        if((not in_range(xyz[0], min_x, np.max(range_x))) or (not in_range(xyz[1], min_y, max_y)) or (not in_range(xyz[2], np.min(range_z), np.max(range_z)))):
            return False
    ...
    # r0 = max(torch.norm(cur_pose_unscaled[:3,-1] - object_center).item(), 0.35)
    r0x = 0.35
    r0y = 0.8
    r0z = 1.2
    
    from copy import deepcopy
    def random_sampling(mean_unc, pop_size=300, num_candidates_to_select=10, angle_diff_th=20, poses_history=[]):
        # breakpoint()
        ...
        # pop = uniform_spherical_volume_sample(pop_size, 0.8*r0, 1.2*r0)
        pop = ellipsoid_volume_sample(pop_size, 0.8*r0x, 1.2*r0x, 0.8*r0y, 1.2*r0y, 0.8*r0y, 1.2*r0y )

        pop_c2w = list(map(lambda x : get_c2w_from_pos(x + object_center.numpy(), object_center.numpy(), True), pop))
        poses_history.append(deepcopy(pop_c2w))


        filtered_pop1 = list(filter(
            lambda x :  in_robot_range(x), pop_c2w))
        poses_history.append(deepcopy(filtered_pop1))
        # breakpoint()
        ...
        poses_history.append(deepcopy(filtered_pop2))
        ...
        if mean_unc is None:
            filter_pop_else1 = list(filter(
                lambda x :  in_robot_range(x, torch.matmul(get_flip_transpose_torch_cpu(object_center).to(nerf2ws.device).double(), nerf2ws.double())), pop_c2w))
        ...
        while len(final_candidates) < num_candidates_to_select:
            ...
            if not found:
                ...
                break
        poses_history.append(deepcopy(final_candidates))
    ...
    def move_action_loss(cam):
        ...
        rotmat = torch.matmul(nerf2ws.to(device).double()[:3, :3], torch.matmul(blender_to_opengl.double(), c2w_blender[..., :3, :3].double()).squeeze())
    ...
    def optimize(camera, optimizer, mean_unc, max_epochs=100, vid_path=None, loss_vid_path=None, merge_vid_path=None, poses_history=None):
        imgs = []
        losses = []
        ...
        if poses_history is not None:
            cam_copy = deepcopy(camera)
            cam_copy.change_coordinate_system(blender_coords())
            cam_copy.change_coordinate_system(blender_coords())
            cam_copy.change_coordinate_system(blender_coords())

            mat = cam_copy.extrinsics.view_matrix().detach().cpu()
    ...
    min_loss = np.inf
    history = []
    initial_poses, mean_unc = random_sampling(mean_unc, poses_history=history)
    ...
    for i, initial_pose in enumerate(initial_poses):
        ...
        loss, cam  = optimize(camera, optimizer, mean_unc, extra_args["epochs"], f"{output_dump_dir}/pose{i+1}_vid.avi", f"{output_dump_dir}/pose{i+1}_vid_loss.avi", f"{output_dump_dir}/pose{i+1}_merged.avi", poses_history=history)
    ...
    pickle.dump(history, open(f"{output_dump_dir}/poses_history.pkl", 'wb'))
    
    best_move_loss = 0.5*move_action_loss(best_cam)
    ...
    return torch.inverse(mat_c2w), min_loss, mean_unc, best_image, best_move_loss
...
if __name__ == '__main__':
    ...
    model_name, object_center, robot_poses, nerf2ws_base = inputs
    ranges = pickle.load(open(args.range_path, 'rb'))

    nerf2ws_base = torch.tensor(nerf2ws_base, dtype=torch.double)


    flip_possible = False
    if not args.no_flip:
        flip_possible = is_flip_possible(args.grasp_data_dir, model_name, args.iter, args.score_th)
        print("Flip Possible : ", flip_possible)

    pipelines = get_pipelines(model_name)
    # bp()
    if flip_possible and args.flip:
        output_dump_dir = f'{args.output_dump_dir}/flip'
        nerf2ws_flip = torch.matmul(get_flip_transpose_torch_cpu(object_center).double(), nerf2ws_base.double())
        nbp_flip, loss_flip, mean_unc_flip, best_image_flip, best_move_loss_flip = main(output_dump_dir, ranges, pipelines, object_center, robot_poses, None, nerf2ws_flip, args.move_lambda, args.unc_lambda)
        cv2.imwrite(f"{output_dump_dir}/best_image.png", cv2.cvtColor((best_image_flip*255).astype(np.uint8), cv2.COLOR_BGR2RGB))
        pickle.dump(nbp_flip.numpy(), open(args.pkl_output_path, 'wb'))
        pickle.dump(nbp_flip, open(f"{args.output_dump_dir}/best_pose.pkl", 'wb'))
        pickle.dump(get_flip_transpose_np(object_center), open(f"{args.output_dump_dir}/flip_transform.pkl", 'wb'))
        pickle.dump(best_move_loss_flip + 1, open(f"{args.output_dump_dir}/action_cost.pkl", 'wb'))
        exit(0)
    
    output_dump_dir = f'{args.output_dump_dir}/no_flip'
    nbp, loss, mean_unc, best_image_no_flip, best_move_loss_no_flip = main(output_dump_dir, ranges, pipelines, object_center, robot_poses, None, nerf2ws_base, args.move_lambda, args.unc_lambda)
    cv2.imwrite(f"{output_dump_dir}/best_image.png", cv2.cvtColor((best_image_no_flip*255).astype(np.uint8), cv2.COLOR_BGR2RGB))

    open(f"{args.output_dump_dir}/mean_unc.txt", 'w').write(str(mean_unc))
    if not flip_possible:
        pickle.dump(nbp.numpy(), open(args.pkl_output_path, 'wb'))
        pickle.dump(nbp, open(f"{args.output_dump_dir}/best_pose.pkl", 'wb'))
        pickle.dump(None, open(f"{args.output_dump_dir}/flip_transform.pkl", 'wb'))
        pickle.dump(best_move_loss_no_flip, open(f"{args.output_dump_dir}/action_cost.pkl", 'wb'))
        exit(0)
    
    output_dump_dir = f'{args.output_dump_dir}/flip'
    nerf2ws_flip = torch.matmul(get_flip_transpose_torch_cpu(object_center).double(), nerf2ws_base.double())
    nbp_flip, loss_flip, mean_unc_flip, best_image_flip, best_move_loss_flip = main(output_dump_dir, ranges, pipelines, object_center, robot_poses, mean_unc, nerf2ws_flip, args.move_lambda, args.unc_lambda)
    cv2.imwrite(f"{output_dump_dir}/best_image.png", cv2.cvtColor((best_image_flip*255).astype(np.uint8), cv2.COLOR_BGR2RGB))

    loss_flip = loss_flip + args.flip_lambda
    
    losses = [loss, loss_flip]
    # bp()
    open(f"{args.output_dump_dir}/losses.txt", 'w').write(str(losses))
