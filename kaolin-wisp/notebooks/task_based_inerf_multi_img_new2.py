...
for i in range(6):
    ...
    start_pose_fmt = get_se3_pose(get_pose_6d(cvt_pose_se3(torch.tensor(start_pose))))
    start_pose_fmts.append(start_pose_fmt)
    im_start = get_scipy_image(start_pose_fmt)
...
updated_pose_fmt = cvt_pose_se3(torch.tensor(updated_pose))
...
def optimize_pose(methods, tol = 1e-4):
    ...
    for method in methods:
        ...
        for i in range(len(start_poses)):
            ...
            # im = get_scipy_image(get_se3_pose(new_pose_6d))
            ...
            fn_min = lossfn_combined_scipy(solution) 
