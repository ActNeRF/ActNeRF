...
# %%
def main(output_dump_dir, ranges, model_name, object_center, robot_pose):
...
if __name__ == '__main__':
    ...
    print(inputs[0])
    print(inputs[1])
    next_best_pose = main(output_dump_dir, ranges, *inputs)
    pickle.dump(next_best_pose.detach().cpu().numpy(), open(pkl_output_path, 'wb'))
    pickle.dump(next_best_pose.detach().cpu().numpy(), open(f"{output_dump_dir}/best_pose.pkl", 'wb'))
