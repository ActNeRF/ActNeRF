...
# %%
def main(output_dump_dir, ranges, model_name, object_center, robot_pose, anygrasp_data_dir, flip):
    if flip:
    ...
    def random_sampling(pop_size=300, num_candidates_to_select=10, angle_diff_th=40):
    ...
    # %%
    def optimize(camera, optimizer, max_epochs=100, vid_path=None, loss_vid_path=None, merge_vid_path=None):
        imgs = []
        losses = []
    ...
    for i, initial_pose in enumerate(initial_poses):
        ...
        loss, cam  = optimize(camera, optimizer, extra_args["epochs"], f"{output_dump_dir}/pose{i+1}_vid.avi", f"{output_dump_dir}/pose{i+1}_vid_loss.avi", f"{output_dump_dir}/pose{i+1}_merged.avi")
...
if __name__ == '__main__':
    ...
    next_best_pose = main(args.output_dump_dir, ranges, *inputs, args.anygrasp_data_dir, False)
    # bp()
    # next_best_pose_flip = main(args.output_dump_dir, ranges, *inputs, args.anygrasp_data_dir, True)
    # bp()

    pickle.dump(next_best_pose.detach().cpu().numpy(), open(args.pkl_output_path, 'wb'))
    pickle.dump(next_best_pose.detach().cpu().numpy(), open(f"{args.output_dump_dir}/best_pose.pkl", 'wb'))
