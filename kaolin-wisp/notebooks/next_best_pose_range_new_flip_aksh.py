...
def main(output_dump_dir, ranges, pipelines, object_center, robot_poses, mean_unc, nerf2ws, move_lambda, unc_lambda):
    ...
    r0 = max(np.mean(np.array([torch.norm(cur_pose_unscaled[:3,-1] - object_center).item(), torch.norm(cur_pose2_unscaled[:3, -1] - object_center).item()])), 0.35)
    print("robo pose dist", torch.norm(cur_pose_unscaled[:3,-1] - object_center).item(), torch.norm(cur_pose2_unscaled[:3, -1] - object_center).item())
    
    from copy import deepcopy
    def random_sampling(mean_unc, pop_size=300, num_candidates_to_select=10, angle_diff_th=40, poses_history=[]):
        # if flip:
