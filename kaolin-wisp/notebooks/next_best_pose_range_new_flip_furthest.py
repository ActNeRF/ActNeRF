...
def main(output_dump_dir, ranges, pipelines, object_center, robot_poses, mean_unc, nerf2ws, move_lambda, unc_lambda, prev_poses):
    ...
    r0 = max(np.mean(np.array([torch.norm(cur_pose_unscaled[:3,-1] - object_center).item(), torch.norm(cur_pose2_unscaled[:3, -1] - object_center).item()])), 0.25)
    ...
    def furthest_view_sampling(mean_unc, pop_size=500, num_candidates_to_select=10, angle_diff_th=40):
        ...
        filtered_pop_c2w = list(filter(
            lambda x: in_robot_range(x) and not object_close_to_img_boundary(camera_obj_from_c2w(x), fraction=0.1),
            pop_c2w
        ))
        filtered_pop_c2w = np.array([np.array(elem) for elem in filtered_pop_c2w])
        print(f"Remaining {len(filtered_pop_c2w)} cands")

        prev_centers = [c2w[:3,-1] * translation_scale.numpy() + translation_offset.numpy() for c2w in prev_poses]
        spherical_distances = list(map(lambda x: avg_sph_dist(prev_centers, x), filtered_pop_c2w[:,:3,-1]))
        furtherst_pose_idx = np.argmax(spherical_distances)
        furthest_pose = filtered_pop_c2w[furtherst_pose_idx]
        print(f"scaled center at {furthest_pose[:3,-1]}")
        print(f"unscaled center at {(furthest_pose[:3,-1] - translation_offset.numpy()) / translation_scale.numpy()}")

        # breakpoint()
        return torch.tensor(furthest_pose), None
    ...
    best_cam = None
    min_loss = np.inf
    initial_pose, mean_unc = furthest_view_sampling(mean_unc)
    ...
    camera = gen_camera(torch.clone(initial_pose).detach(), extra_args['focal'], W, H, z_far, extra_args)
    move_loss = 0.5*move_action_loss(camera)
    im = get_image(camera, pipelines[0])[0]
    mat_c2w = initial_pose[None, :]
    mat_c2w[..., :3, 3] -= translation_offset
    mat_c2w[..., :3, 3] /= translation_scale

    # breakpoint()
    return torch.inverse(mat_c2w), min_loss, mean_unc, im, move_loss
...
if __name__ == '__main__':
    ...
    parser.add_argument('-pc', '--prev-poses-path', type=str, required=True)
    ...
    prev_poses = pickle.load(open(args.prev_poses_path, 'rb'))
    ...
    if not flip_possible:
        ...
        nbp, loss, mean_unc, best_image_no_flip, move_loss_no_flip = main(output_dump_dir, ranges, pipelines, object_center, robot_poses, None, nerf2ws_base, args.move_lambda, args.unc_lambda, prev_poses)
        ...
        pickle.dump(move_loss_no_flip, open(f"{args.output_dump_dir}/action_cost.pkl", 'wb'))
        ...
        nbp_flip, loss_flip, mean_unc_flip, best_image_flip, move_loss_flip = main(output_dump_dir, ranges, pipelines, object_center, robot_poses, None, nerf2ws_flip, args.move_lambda, args.unc_lambda, prev_poses)
        ...
        pickle.dump(move_loss_flip + 1, open(f"{args.output_dump_dir}/action_cost.pkl", 'wb'))
