...
def batch_forward_only(pipelines, rays, batch_size=4000):
    ...
    for patch_rays in rays.split(batch_size):
            ...
            all_epis = torch.zeros((len(pipelines), batch_size, 1), dtype=float)
            for i, pipeline in enumerate(pipelines):
                rb = render_pipeline(pipeline, patch_rays, None)
                all_rgbs[i,:,:] = rb.rgb
                all_epis[i, :, :] = rb.epistemic2

            avg_epis = torch.mean(all_epis, axis=0)
            avg_epis[avg_epis == 0] = 1
            unc_loss2 = -torch.mean((1-avg_epis)**2)
            unc_loss = -torch.mean(torch.var(all_rgbs, axis = 0))
            total_loss += unc_loss.item() + unc_loss2.item()
    return total_loss
...
def batch_backwards(pipelines, rays:Rays, nerf_mean_unc, unc_lambda, batch_size=4000):
    ...
    for patch_rays in rays.split(batch_size):
            ...
            avg_epis = torch.mean(all_epis, axis=0)
            avg_epis[avg_epis == 0] = 1
            unc_loss2 = -torch.mean((1-avg_epis)**2)  / np.abs(nerf_mean_unc)
            unc_loss1 =  (-torch.mean(torch.var(all_rgbs, axis = 0)) / np.abs(nerf_mean_unc))
            unc_loss = unc_lambda * (unc_loss1 + unc_loss2)
