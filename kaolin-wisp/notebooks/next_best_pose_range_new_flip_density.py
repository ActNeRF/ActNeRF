...
def batch_forward_only(pipelines, rays, batch_size=4000):
    ...
    for patch_rays in rays.split(batch_size):
            all_epis = torch.zeros((len(pipelines), batch_size, 3), dtype=float)
            ...
            unc_loss = -torch.mean((1-avg_epis)**2)
...
def batch_backwards(pipelines, rays:Rays, nerf_mean_unc, unc_lambda, batch_size=4000):
    ...
    for patch_rays in rays.split(batch_size):
            ...
            unc_loss = unc_lambda * ( -torch.mean((1-avg_epis)**2))  / np.abs(nerf_mean_unc)
            # unc_loss = unc_lambda * (-torch.mean(all_epis))
            total_loss += unc_loss.item()
...
def main(output_dump_dir, ranges, pipelines, object_center, robot_poses, mean_unc, nerf2ws, move_lambda, unc_lambda):
    ...
    def in_range(x, xmin, xmax):
        # print(xmin, xmax)
        return ((x >= xmin + 0.05) and (x <= xmax - 0.05))
