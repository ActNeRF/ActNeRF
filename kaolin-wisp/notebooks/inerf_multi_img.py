...
parser.add_argument('-tp1', '--target-image-path1', type=str, default='', help='Path to target image.')
parser.add_argument('-tp2', '--target-image-path2', type=str, default='', help='Path to target image.')
...
W, H = 200, 200
target_image_np1 = cv2.resize(cv2.imread(args.target_image_path1, cv2.IMREAD_UNCHANGED), (W, H), interpolation=cv2.INTER_AREA)
target_image_np1 = cv2.cvtColor(target_image_np1, cv2.COLOR_BGRA2RGBA)

target_image_np2 = cv2.resize(cv2.imread(args.target_image_path2, cv2.IMREAD_UNCHANGED), (W, H), interpolation=cv2.INTER_AREA)
target_image_np2 = cv2.cvtColor(target_image_np2, cv2.COLOR_BGRA2RGBA)
...
target_pose = torch.tensor(pickle.load(open(args.target_c2w_path1, 'rb')))
target_pose2 = torch.tensor(pickle.load(open(args.target_c2w_path2, 'rb')))
...
target_pose2[..., :3, 3] *= translation_scale.numpy()
target_pose2[..., :3, 3] += translation_offset.numpy()
...
target_image_flatten1 = np.reshape(target_image_np1, [-1, 4]) / 255.0
target_image_flatten1 = torch.from_numpy(target_image_flatten1).float().to(device=extra_args['device'])

target_image_flatten2 = np.reshape(target_image_np2, [-1, 4]) / 255.0
target_image_flatten2 = torch.from_numpy(target_image_flatten2).float().to(device=extra_args['device'])
...
def lossfn_combined_scipy(p):
    se3_pose1 = get_se3_pose(p)
    se3_pose2 = torch.linalg.inv(target_pose2) @ target_pose @ se3_pose1

    loss1 = lossfn2(se3_pose1, target_image_flatten1, rewrite=True, opt_all=True)
    loss2 = lossfn2(se3_pose2, target_image_flatten2, rewrite=True, opt_all=True)
...
base_rotation = target_pose[:3, :3].numpy()
...
for i in range(len(posses)):
    ...
    # im_target1 = cv2.cvtColor(target_image_np1, cv2.COLOR_RGBA2BGR)
    # im_target2 = cv2.cvtColor(target_image_np2, cv2.COLOR_RGBA2BGR)

    # im_end1 = get_scipy_image(get_se3_pose(get_pose_6d(cvt_pose_se3(ans_pose))))
    # im_end2 = get_scipy_image(torch.linalg.inv(target_pose2) @ target_pose @ torch.tensor(get_se3_pose(get_pose_6d(cvt_pose_se3(ans_pose)))))
    # cv2.imwrite('zimg1.png',cv2.absdiff(im_target1, cv2.cvtColor(im_end1, cv2.COLOR_BGR2RGB)))
    # cv2.imwrite('zimg2.png',cv2.absdiff(im_target2, cv2.cvtColor(im_end2, cv2.COLOR_BGR2RGB)))

    # im_loss1 = lossfn_combined_scipy(get_pose_6d(cvt_pose_se3(ans_pose)))

    # im_start1 = get_scipy_image(get_se3_pose(get_pose_6d(cvt_pose_se3(start_pose))))
    # cv2.imwrite('zimg2.png',cv2.cvtColor(im_start1, cv2.COLOR_BGR2RGB))
    # bp()
...
im_target1 = cv2.cvtColor(target_image_np1, cv2.COLOR_RGBA2RGB)

start_diff = cv2.absdiff(im_start, im_target1)
end_diff = cv2.absdiff(im_end, im_target1)
...
im_start = get_scipy_image(torch.linalg.inv(target_pose2) @ target_pose @ get_se3_pose(new_pose_6d))
im_end = get_scipy_image(torch.linalg.inv(target_pose2) @ target_pose @ get_se3_pose(poses[idx]))
im_target2 = cv2.cvtColor(target_image_np2, cv2.COLOR_RGBA2RGB)

start_diff = cv2.absdiff(im_start, im_target2)
end_diff = cv2.absdiff(im_end, im_target2)

cv2.imwrite(f'{args.output}/image_start2.png', cv2.cvtColor(im_start, cv2.COLOR_RGB2BGR))
cv2.imwrite(f'{args.output}/image_end2.png', cv2.cvtColor(im_end, cv2.COLOR_RGB2BGR))

cv2.imwrite(f'{args.output}/diff_start2.png', cv2.cvtColor(start_diff, cv2.COLOR_RGB2BGR))
cv2.imwrite(f'{args.output}/diff_end2.png', cv2.cvtColor(end_diff, cv2.COLOR_RGB2BGR))
