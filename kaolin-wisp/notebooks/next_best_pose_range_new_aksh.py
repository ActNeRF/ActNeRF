...
# %%
def get_image(cam: Camera, pipeline) -> Tuple[np.ndarray, np.ndarray]:
...
# %%
def main(output_dump_dir, ranges, model_name, object_center, robot_poses, anygrasp_data_dir, flip, move_lambda, unc_lambda):
    if flip:
        output_dump_dir = f'{output_dump_dir}_flip'
    else:
        output_dump_dir = f'{output_dump_dir}_noflip'
    ...
    def random_sampling(pop_size=500, num_candidates_to_select=10, angle_diff_th=40):
        ...
        mean_unc = np.mean([x[0] for x in candidates_sorted])
        return final_candidates, mean_unc
    
    def get_cam_c2w_in_nerf_world_coordinates(cam):
    ...
    def stopping_cond(losses, cam, best_loss):
        ...
        if early_stopping(losses, best_loss):
            ...
            return True
        # camc = copy.deepcopy(cam)
        c2w = get_cam_c2w_in_nerf_world_coordinates(cam)
    ...
    def move_action_loss(cam):
        ...
        xyz_scaled = torch.matmul(blender_to_opengl, c2w_blender[..., :3, 3].transpose(0, 1)).squeeze()
        ...
        rotmat = torch.matmul(blender_to_opengl, c2w_blender[..., :3, :3]).squeeze()
    ...
    # subprocess.run(f"cd /home/saptarshi/dev/kaolin-wisp/notebooks && python generate_depth_image.py {model_name} {anygrasp_data_dir}", shell=True)
    # subprocess.run(f"cd /home/saptarshi/dev/anygrasp/anygrasp_sdk/grasp_detection && python demo.py --checkpoint_path log/checkpoint_detection.tar --data-dir {anygrasp_data_dir}", shell=True)
    
    # grasp = pickle.load(open(f"{anygrasp_data_dir}/grasp.pkl", 'rb'))
    # cam_pose = pickle.load(open(f"{anygrasp_data_dir}/c2w.pkl", 'rb'))
    ...
    # construct a 2D rectangular maskk from p1,p2 as endpoints
    ...
    #     images[i] = get_image(camera, pipeline)[0]
    # unc = np.var(images, axis=0).mean(axis=2)[int(p1[1]):int(p2[1]), int(p1[0]):int(p2[0])]
    # unc_std = unc.mean() ** 0.5
    # print('unc std', unc_std)
    # im = cv2.cvtColor((images[0]*255).astype(np.uint8), cv2.COLOR_BGR2RGB)
    # im[int(p1[1]):int(p2[1]), int(p1[0]):int(p2[0])] = 0
    ...
    best_cam = None
    min_loss = np.inf
    initial_poses, mean_unc = random_sampling()
...
if __name__ == '__main__':
    ...
    parser.add_argument('-a', '--anygrasp-data-dir', type=str, required=True)
    ...
    next_best_pose = main(args.output_dump_dir, ranges, *inputs, args.anygrasp_data_dir, False, args.move_lambda, args.unc_lambda)
    # bp()
    # next_best_pose_flip = main(args.output_dump_dir, ranges, *inputs, args.anygrasp_data_dir, True)
    # bp()

    pickle.dump(next_best_pose.numpy(), open(args.pkl_output_path, 'wb'))
    pickle.dump(next_best_pose.numpy(), open(f"{args.output_dump_dir}/best_pose.pkl", 'wb'))
