...
def parse_args():
    ...
    density_map_group.add_argument('--sample_density', type=int, default=1000)
...
def get_all_side_points_new(side_corner, di, sample_density, num_samples_per_side):
    points = []
    init_pos = side_corner.copy()
    cur_pos = side_corner.copy()

    for i in range(num_samples_per_side[0]):
        cur_pos[1] = init_pos[1]
        for j in range(num_samples_per_side[1]):
            cur_pos[2] = init_pos[2]
            for k in range(num_samples_per_side[2]):
                points.append(cur_pos.copy())
                if di == 2:
                    break
                cur_pos[2] += sample_density
            if di == 1:
                break
            cur_pos[1] += sample_density
        if di == 0:
            break
        cur_pos[0] += sample_density
...
for id, (corner, dir) in enumerate(zip(side_corners, dirs)):
    ...
    total_points = []
    num_points = (side_l * sample_density).astype(int)

    for i in range(-50, 50):
        corner_new = corner.copy()
        corner_new[dir] += i*(delta/50)
        points = get_all_side_points_new(corner_new, dir, sample_density, num_points)
        total_points.append(points)

    total_points =  torch.tensor(total_points).to(device)
    print("total_points", total_points.size())
    ...
    print("density_size", avg_density.size())
    print("max_density", avg_density.max())
    if dir == 0:
        avg_density = avg_density.reshape((num_points[1], num_points[2]))
    elif dir == 1:
        avg_density = avg_density.reshape((num_points[2], num_points[0]))    
    else:
        avg_density = avg_density.reshape((num_points[0], num_points[1]))    

    import os 
    os.makedirs(model_name, exist_ok=True)

    np.save(f'{model_name}/d_{id}', avg_density.detach().cpu().numpy())
