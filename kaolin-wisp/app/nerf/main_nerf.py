...
def parse_args():
    ...
    trainer_group.add_argument('--save-every', type=int, default=(-1 if is_interactive() else 25),
                               help='Save the model at every N epoch.')
    trainer_group.add_argument('--render-tb-every', type=int, default=(-1 if is_interactive() else 25),
                               help='Render every N epochs')
    trainer_group.add_argument('--log-tb-every', type=int, default=25,  # TODO (operel): move to logging
...
def load_neural_pipeline(args, dataset, device) -> Pipeline:
    ...
    if args.pretrained:
        import glob
        model_path = glob.glob(args.pretrained)[0]
